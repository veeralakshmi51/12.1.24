...
export const updateStaffDetails = (id: string, data: any,organization:string) => async (
  dispatch: any
) => {
  dispatch(isLoading());
  ...
  try {
    ...
    const response = await axios.put(`${baseURL}/staff/update/${id}`, jsonData,{
     headers:{
...
// export const getStaffDetails = (id: string, organization: string) => async (
//   dispatch: any
// ) => {
//   dispatch(isLoading());
//   try {
//     const response = await axios.get(`${baseURL}/staff/get/${id}`);
//     console.log('API response', response.data);
//     if (response.data.message.code === successCode) {
//       dispatch(getStaffSuccess(response.data.data));
//     } else {
